...
$bdepth = \beta_0 + \beta_1 \times blength$
...
* the observed beak depth is `r finch473 %>% pull(bdepth)`
* the observed beak length is `r finch473 %>% pull(blength)`
* the fitted value is `r fortis_1975_example %>% filter(band == 473) %>% pull(.fitted) %>% round(digits = 2)`
* the error is `r fortis_1975_example %>% filter(band == 473) %>% pull(.resid) %>% round(digits = 2)`
