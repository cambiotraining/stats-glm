...
#| fig-cap: "Beak classification without class information"
# filter for early time points
early_finches <- finches_2020 %>% 
  filter(timepoint == "early") %>% 
  mutate(pointed_beak = if_else(str_detect(class, "blunt"), 0, 1))

p1 <- ggplot(early_finches, aes(x = blength, y = pointed_beak)) +
  geom_point() +
  labs(x = "beak length (mm)",
       y = "probability of having a pointed beak")

p2 <- ggplot(early_finches, aes(x = blength, y = pointed_beak)) +
  geom_point() +
...
We'll use a _logistic regression_ to analyse these data. We'll cover that in more detail in a later chapter, but for now it's sufficient to realise that one of the ways we could model these data could look like this:

```{r}
#| echo: false
#| message: false
#| label: fig-beak_class_glm
...
Now, what happened behind the scenes here is that the generalised linear model has taken the linear predictor and transformed it using the _logit_ function. This links the linear predictor to the non-linear _outcome_ (or response). Conversely, we can look at the fitted values of the model (before they get transformed). They look like this:

```{r}
#| echo: false
#| message: false
#| label: fig-beak_class_glm_fitted
#| fig-cap: "Fitted values for beak classification"
...
#| echo: false
#| message: false
#| label: fig-beak_class_residuals
#| fig-cap: "Residuals before and after link function"
glm_early <- glm(pointed_beak ~ blength,
                 family = binomial(link = "logit"),
                 data = early_finches)
