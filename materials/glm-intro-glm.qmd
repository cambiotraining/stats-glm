...
1. Transform our data and use a normal linear model
2. Use a GLM
:::

The first option, transforming our data, seems like a useful option and can work. It keeps things familiar (we'd still use a standard linear model) and all is well with the world. Up to the point of interpreting the data. If we, for example, log-transform our data, how do we interpret this? After all, the outcome of the linear model is directly related to the predicted response variable. Furthermore, although transforming the data may normalise the residuals, it also changes the ratio scale properties of the measured variables [@stevens1946].

The second option would be to use a generalised linear model. Here we keep the data the same, but we _transform the linear predictor_, enabling us to drop the assumption that the response variable comes from a normal distribution. This transformation is done using a link function, which maps the non-linear response to a linear one.
...
Two species, _Geospiza fortis_ and _G. scandens_. There are measurements that are split by a uniquely timed event: a particularly strong El Niño event in 1983 changed the vegetation and food supply of the finches, allowing F1 hybrids to survive. These measurements are classed as `early` (pre-1983) and `late` (1983 onwards).
...
finches_2020 <- finches_2020 %>% 
  clean_names() %>% 
  rename(class = group) %>% 
  mutate(timepoint = (str_detect(class, "Late")),
         timepoint = if_else(timepoint == FALSE, "early", "late"),
         timepoint = factor(timepoint))
...
#| fig-cap: "Beak classification pre- and post El Niño"
finches_2020 %>% 
